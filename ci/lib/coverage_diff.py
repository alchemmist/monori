...
def load_baseline(path: Path | None) -> dict[str, float]:
    """Load an optional main-branch baseline."""
    ...
    for name in STACK_PATHS:
        value = stacks.get(name)
...
def resolve_pull_request(github: GitHubAPI, head_sha: str) -> int:
    ...
    for item in pulls:
        ...
        if pull.get("state") == "open" and isinstance(number, int) and not isinstance(number, bool):
            return number
    message = f"No open pull request found for {head_sha}"
    raise RuntimeError(message)
...
def coverage_job_passed(github: GitHubAPI, run_id: int) -> bool:
    ...
    response = object_value(
        github.request("GET", f"/actions/runs/{run_id}/jobs?per_page=100"), "workflow jobs"
    )
...
def publish_report(
    ...
    *,
    ...
    pr_number = resolve_pull_request(github, head_sha)
    workflow_passed = coverage_job_passed(github, run_id)
    try:
        report = load_artifact(artifact, head_sha, pr_number)
