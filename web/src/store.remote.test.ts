        ...
        useStore.setState((state) => ({
            snapshot: {
                ...
                categories: [
                    ...
                    { id: 5, groupId: 2, name: "Rent", keywords: "", sort: 2, archived: false },
                ],
            },
        }));

        await useStore.getState().moveCategory(4, 3, [4, 5], goal);

        expect(patch).toHaveBeenCalledExactlyOnceWith(4, { groupId: 3, ...goal });
        expect(reorder).toHaveBeenCalledExactlyOnceWith([4, 5]);
        expect(snap().categories[0]).toMatchObject({ groupId: 3, ...goal });
        expect(snap().categories[1]).not.toMatchObject(goal);
    });
