...
import type { BudgetMonth, BudgetYearResult } from "./budget.js";
import type { Snapshot } from "../types.js";

const buildYearResult = (byCategory: Map<number, BudgetMonth[]>): BudgetYearResult => ({
    year: 2024,
    byCategory,
...
describe("categoryYearMatrix", () => {
    ...
    it("keeps income-category data scoped to the selected year", () => {
        const withPriorIncome = buildSnapshot({
            ...snapshot,
            transactions: [
...
describe("categoryTotals", () => {
    ...
    it("drops uncategorized rows and transfer legs", () => {
        const withIgnored = buildSnapshot({
            ...snapshot,
            transactions: [
                ...snapshot!.transactions,
...
describe("accountBalances", () => {
    ...
    it("treats a missing accounts list as empty", () => {
        expect(accountBalances(buildSnapshot({ accounts: [], transactions: [] })).size).toBe(0);
    });
});
...
describe("yearTotals edge cases", () => {
    it("sorts years ascending and handles a zero-income year", () => {
        const monthly: ReturnType<typeof monthlySeries> = [
...
describe("txStats keeps the first of tied-largest expenses", () => {
    ...
    it("excludes an outflow whose categoryId no longer resolves", () => {
        const withDangling = buildSnapshot({
            ...snap,
            transactions: [
                ...snap.transactions,
                {
...
describe("disciplineMatrix mechanics", () => {
    ...
    const yearResult = buildYearResult(
        new Map([
            [20, cat20],
            [21, cat21],
    ...
    it("returns a null hit rate when no month is active", () => {
        const idleOnly = buildYearResult(new Map([[22, zeros()]]));
...
describe("transfer legs are invisible to every income/expense total", () => {
    ...
    const withTransfer = buildSnapshot({
        ...snapshot,
        transactions: [
            ...snapshot!.transactions,
            {
    ...
    it("keeps the budget engine from spending an envelope on a transfer", () => {
        const range = (value: Snapshot) => computeRange(value, 2024, 2024);
        expect(range(withTransfer)).toEqual(range(snapshot));
    });

    it("still moves the money between the two account balances", () => {
        const accounts = withTransfer.accounts.map((account) => ({
            ...account,
            openingBalance: 0,
        }));
        if (!accounts.some((account) => account.id === 2)) {
            accounts.push({ ...accounts[0]!, id: 2, openingBalance: 0 });
        }
        const baseline = accountBalances({ ...snapshot, accounts });
        const balances = accountBalances({ ...withTransfer, accounts });
        expect(balances.get(1)! - (baseline.get(1) ?? 0)).toBe(-50_000_00);
        expect(balances.get(2)! - (baseline.get(2) ?? 0)).toBe(50_000_00);
    });
});
...
describe("year-scoped aggregators ignore a dangling category", () => {
    const withDangling = (base: Snapshot): Snapshot =>
        buildSnapshot({
            ...base,
            transactions: [
                ...base.transactions,
