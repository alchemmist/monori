...
export function isTransfer<T extends { transferId?: string | null | undefined }>(
    t: T,
): t is T & { transferId: string } {
 ...
 */
export type TransferTableItem =
    | { kind: "tx" | "leg"; key: string; tx: Transaction }
    | {
          kind: "transfer";
          key: string;
          transferId: string;
...
export function transferDates(item: {
    out: Pick<Transaction, "date">;
    in: Pick<Transaction, "date">;
}) {
