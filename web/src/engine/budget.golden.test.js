...
const snapshot = JSON.parse(readFileSync(path.join(OUT, "snapshot.json"), "utf8"));
const reference = JSON.parse(readFileSync(path.join(OUT, "reference.json"), "utf8"));

const kop = (rub) => Math.round(rub * 100);
const catIdByName = new Map(snapshot.categories.map((c) => [c.name, c.id]));
const results = computeRange(snapshot, 2020, 2027);
...
describe("engine reproduces the spreadsheet", () => {
