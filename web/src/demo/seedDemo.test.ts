...
beforeEach(() => {
    vi.clearAllMocks();
    let nextId = 1000;
    vi.mocked(api.createAccount).mockImplementation(async () => ({ id: nextId++ }));
    vi.mocked(api.createGroup).mockImplementation(async () => ({ id: nextId++ }));
    vi.mocked(api.createCategory).mockImplementation(async () => ({ id: nextId++ }));
    vi.mocked(api.patchAccount).mockResolvedValue({});
    vi.mocked(api.bulkBudgets).mockResolvedValue({});
    vi.mocked(api.importCommit).mockResolvedValue({ inserted: 3, skipped: 1 });
    vi.mocked(api.createTransfer).mockResolvedValue({ transferId: "demo" });
    vi.mocked(api.transactions).mockResolvedValue({ rows: [], total: 0 });
    vi.mocked(api.hiddenTx).mockResolvedValue({ rows: [], total: 0 });
});
...
describe("seedDemoData on an empty account", () => {
    it("creates every account, group and category, then budgets and imports", async () => {
        ...
        expect(vi.mocked(api.bulkBudgets).mock.calls[0]![0]).toHaveLength(
            demoSnapshot.budgets.length,
        );
    ...
    it("creates a transfer for every demo transfer pair", async () => {
        ...
        const body = vi.mocked(api.createTransfer).mock.calls[0]![0];
...
describe("seedDemoData resuming a partial run", () => {
    ...
    it("gives a new demo account a unique name when the name already exists", async () => {
        ...
        expect(api.createAccount).toHaveBeenCalledTimes(demoSnapshot.accounts.length);
        for (const call of vi.mocked(api.createAccount).mock.calls) {
    ...
    it("does not recreate a transfer whose pair is already present", async () => {
        ...
        vi.mocked(api.createAccount).mockImplementation(async ({ bankRef }) => {
        ...
        for (const { out, inn } of pairs.values()) {
            if (!out || !inn) throw new Error("incomplete demo transfer pair");
            existingRows.push(
                makeTx(out.id, {
    ...
    it("pages through visible and hidden transactions", async () => {
        vi.mocked(api.snapshot).mockResolvedValue(emptySnapshot());
        vi.mocked(api.transactions)
            .mockResolvedValueOnce({ rows: [makeTx(1), makeTx(2)], total: 3 })
            .mockResolvedValueOnce({ rows: [makeTx(3)], total: 3 });
        vi.mocked(api.hiddenTx).mockResolvedValueOnce({ rows: [makeTx(4)], total: 1 });
