...
import type { BudgetCell, Snapshot, Transaction, Transfer } from "../types.js";

const R = (rub: number) => Math.round(rub * 100); // rubles -> kopecks
...
type CategorySeed = [number, string, string, number, number, [number, number], string[]];
...
function build(): Snapshot {
    ...
    const categoryId = (name: string): number => {
        ...
        if (id === undefined) throw new Error(`unknown demo category: ${name}`);
        return id;
    };
    ...
    for (const [y, m] of months) {
        ...
        for (const [groupId, name, , budgetRub] of CATS) {
            ...
            budgets.push({ categoryId: categoryId(name), year: y, month: m, amount: R(budgetRub) });
        }
