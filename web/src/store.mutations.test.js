...
describe("store remote mutations", () => {
    ...
    const refreshCount = () => useStore.getState().load.mock.calls.length;
    ...
    it("reloads after every bank-connection call", async () => {
        ...
        const remove = vi.spyOn(api, "deleteConnection").mockResolvedValue({});
        ...
        const cancel = vi.spyOn(api, "cancelConnectionSync").mockResolvedValue({});
        const s = useStore.getState();

        await expect(s.createConnection({ bank: "tbank" })).resolves.toEqual({ id: 7 });
        expect(create).toHaveBeenCalledExactlyOnceWith({ bank: "tbank" });
        expect(refreshCount()).toBe(1);
