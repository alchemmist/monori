...
const perRowColor = (field: string) => (props: BarShapeProps) => {
    const payload: unknown = props.payload;
    const fill: unknown =
        typeof payload === "object" && payload != null && field in payload
            ? (Reflect.get(payload, field) as unknown)
            : undefined;
    return <Rectangle {...props} fill={typeof fill === "string" ? fill : undefined} />;
};
...
function categoryChartData(
    ...
    const named = rows.map((r) => ({
        ...
        label:
            r.id != null && (seen.get(r.name) ?? 0) > 1
                ? `${r.name} · ${r.groupId == null ? "Other" : groupName.get(r.groupId)}`
                : r.name,
    }));
    const blankAfter = +year === now.getFullYear() ? now.getMonth() : 11;
    const data = MONTHS_SHORT.map((mo, m) => {
        const row: Record<string, string | number | null> = { month: mo };
...
const truncateMid = (s: string, max = 24) => {
...
export default function AnalyticsPage({
    ...
    if (!snapshot) throw new Error("Analytics requires a loaded snapshot");
    ...
    const discipline = useMemo(() => {
        ...
        if (!res) throw new Error(`Missing budget result for ${year}`);
        ...
        <div className="fade-in dash-section">
            ...
            <div className="kpi-row">
                ...
                <Kpi
                    ...
                    color={
                        (discipline.hitRate ?? 0) >= 80
                            ? "var(--m-income)"
                            : (discipline.hitRate ?? 0) >= 60
                              ? "var(--m-warning)"
                              : "var(--m-expense)"
                    }
...
type DisciplineRows = ReturnType<typeof disciplineMatrix>["rows"];
type DisciplineCell = DisciplineRows[number]["cells"][number];
type DisciplineCategory = DisciplineRows[number]["category"];

function DisciplineGrid({ rows, year }: { rows: DisciplineRows; year: string }) {
    if (!rows.length) {
...
function discClass(cell: DisciplineCell) {
...
function discTitle(category: DisciplineCategory, cell: DisciplineCell, m: number) {
