...
const adminTx = (id: number) => ({ ...detail.recentTransactions[0]!, id });
...
describe("AdminPage", () => {
    ...
    const findUserRow = async (email = "person@example.test") =>
        (await screen.findByRole("cell", { name: email })).closest<HTMLElement>("tr")!;
    ...
    it("renders an API failure instead of an empty page", async () => {
        vi.mocked(api.adminOverview).mockRejectedValueOnce(new Error("forbidden"));
        renderUI(<AdminPage />);
        expect(
            await screen.findByText("Failed to load admin data: Error: forbidden"),
        ).toBeInTheDocument();
    });
    ...
    it("renders empty user data, admin and failed sync states", async () => {
        ...
        expect(
            screen
                .getByText("failed-sync@example.test")
                .closest<HTMLElement>("tr")!
                .querySelector<HTMLElement>(".admin-sync")!,
        ).toHaveAttribute("title", "token expired");
        expect(screen.getAllByRole("button", { name: "Delete" })).toHaveLength(1);
        // the delete action sits on the non-admin row, and the admin row has none
        const adminRow = screen.getByText("admin@example.test").closest<HTMLElement>("tr")!;
        const nonAdminRow = screen
            .getByText("failed-sync@example.test")
            .closest<HTMLElement>("tr")!;
    ...
    it("renders every KPI with its exact value and subtitle", async () => {
        ...
        const kpis = within(container.querySelector<HTMLElement>(".admin-kpis")!);
        const kpi = (label: string) => kpis.getByText(label).closest<HTMLElement>(".kpi")!;
        ...
        expect(kpi("Transactions").querySelector<HTMLElement>(".kpi__value")!.textContent).toBe(
            `1${NBSP}234`,
        );
    ...
    it("formats byte sizes across unit and rounding boundaries", async () => {
        ...
        for (const [bytes, want] of cases) {
            vi.mocked(api.adminOverview).mockResolvedValue({ ...overview, dbSizeBytes: bytes });
    ...
    it("formats user-row dates, counts and sync badge exactly", async () => {
        ...
        await screen.findByRole("heading", { name: "Admin" });
        const row = screen
            .getByRole("cell", { name: "person@example.test" })
            .closest<HTMLElement>("tr")!;
        ...
        const badge = row.querySelector<HTMLElement>(".admin-sync")!;
        expect(badge).toHaveTextContent("connected");
        // connected -> income tone dot; lastSync appended via fmtDate
        expect(badge.querySelector<HTMLElement>(".admin-sync__dot")!).toHaveStyle({
    ...
    it("shows an em dash for missing dates and no connection", async () => {
        ...
        expect(row.querySelector<HTMLElement>(".admin-muted")!).toHaveTextContent("—");
    });
    ...
    it("uses the warning tone for a pending sync without a last-sync date", async () => {
        ...
        renderUI(<AdminPage />);
        const badge = (await findUserRow()).querySelector<HTMLElement>(".admin-sync")!;
    ...
    it("resets the delete arming when the pointer leaves the row", async () => {
        ...
        fireEvent.mouseLeave(remove.closest<HTMLElement>("tr")!);
    ...
    it("renders the detail lists, transaction preview and manage tab args", async () => {
        ...
        const { user: events } = renderUI(<AdminPage />);
        await events.click(await findUserRow());
        const detailEl = (await screen.findByText("Recent transactions")).closest<HTMLElement>(
            ".admin-detail",
        )!;
        ...
        expect(
            detailEl.querySelectorAll<HTMLElement>(".admin-detail__col .admin-logins")[2]!.children,
        ).toHaveLength(8);
    ...
    it("caps the recent-transactions preview at five rows", async () => {
        ...
        const { user: events } = renderUI(<AdminPage />);
        await events.click(await findUserRow());
        const table = (await screen.findByText("tx 0")).closest<HTMLElement>("table")!;
        expect(table.querySelectorAll<HTMLElement>("tbody tr")).toHaveLength(5);
    ...
    it("caps recent logins at twelve in the activity card", async () => {
        ...
        const list = (await screen.findByText("Recent logins"))
            ...
            .querySelector<HTMLElement>(".admin-logins")!;
    ...
    it("stops paging once a short transaction page comes back", async () => {
        vi.spyOn(api, "adminUserDetail").mockResolvedValue(detail);
        vi.spyOn(api, "adminUserTransactions").mockResolvedValueOnce([adminTx(1), adminTx(2)]);
    ...
    it("downloads every transaction page from the detail", async () => {
        ...
        vi.spyOn(api, "adminUserTransactions")
            .mockResolvedValueOnce(Array.from({ length: 1000 }, (_, id) => adminTx(id)))
            .mockResolvedValueOnce([adminTx(1001)]);
        const popup = { location: "", close: vi.fn() };
        vi.spyOn(window, "open").mockReturnValue(popup as unknown as Window);
        vi.stubGlobal("URL", {
        ...
        await screen.findByRole("heading", { name: "Admin" });
        await events.click(
            screen.getByRole("cell", { name: "person@example.test" }).closest<HTMLElement>("tr")!,
        );
        await events.click(await screen.findByRole("button", { name: "Full" }));
