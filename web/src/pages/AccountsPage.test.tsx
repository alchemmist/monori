...
const account = (id: number, patch: Partial<Account> = {}): Partial<Account> => ({
    id,
    name: `Account ${id}`,
    type: "card",
    icon: "card",
...
const row = (name: string): HTMLElement => {
    const found = [...document.querySelectorAll<HTMLElement>(".account-row")].find(
        (r) => r.querySelector<HTMLElement>(".account-row__name")!.textContent === name,
    );
    if (!found) throw new Error(`Missing account row: ${name}`);
    return found;
};
const openMenu = async (user: UserEvent, name: string) => {
    await user.click(row(name).querySelector<HTMLElement>(".account-row__actions button")!);
};
...
describe("AccountsPage", () => {
    beforeEach(() => {
        ...
        setPath("/");
        vi.clearAllMocks();
        vi.mocked(api.connectionsAvailable).mockResolvedValue([]);
    });
    ...
    it("lists the demo accounts under the page heading", () => {
        ...
        expect(
            document.querySelectorAll<HTMLElement>(".account-row:not(.account-row_add)"),
        ).toHaveLength(data.accounts.length);
    });
    ...
    describe("balances", () => {
        it("adds each account's transactions to its opening balance", () => {
            ...
            const balance = (name: string) =>
                row(name).querySelector<HTMLElement>(".account-row__balance")!.textContent;
    ...
    describe("tags", () => {
        it("labels an account by type and marks archived ones", () => {
            ...
            const tags = (name: string) =>
                [...row(name).querySelectorAll<HTMLElement>(".tag")].map((t) => t.textContent);
        ...
        it("only calls a connection synced when it really is connected", () => {
            ...
            const connTag = (name: string) => row(name).querySelectorAll<HTMLElement>(".tag")[1];
    ...
    describe("drag and drop reordering", () => {
        ...
        const dragFirstDown = (container: HTMLElement) => {
            const grip = container.querySelector<HTMLElement>(".account-row__grip")!;
            vi.spyOn(
                grip.closest<HTMLElement>(".account-row")!,
                "getBoundingClientRect",
            ).mockReturnValue(DOMRect.fromRect({ height: 40 }));
        ...
        it("gives every row a grip", () => {
            ...
            expect(container.querySelectorAll<HTMLElement>(".account-row__grip")).toHaveLength(2);
        });
        ...
        it("moves a middle account to the end, keeping the others in order", () => {
            ...
            const grip = row("Second").querySelector<HTMLElement>(".account-row__grip")!;
            vi.spyOn(
                grip.closest<HTMLElement>(".account-row")!,
                "getBoundingClientRect",
            ).mockReturnValue(DOMRect.fromRect({ height: 40 }));
        ...
        it("marks only the dragged row and lifts it, shifting the others out of the way", () => {
            ...
            const grip = row("First").querySelector<HTMLElement>(".account-row__grip")!;
            vi.spyOn(
                grip.closest<HTMLElement>(".account-row")!,
                "getBoundingClientRect",
            ).mockReturnValue(DOMRect.fromRect({ height: 40 }));
        ...
        const startDrag = (name: string) => {
            const grip = row(name).querySelector<HTMLElement>(".account-row__grip")!;
            vi.spyOn(
                grip.closest<HTMLElement>(".account-row")!,
                "getBoundingClientRect",
            ).mockReturnValue(DOMRect.fromRect({ height: 40 }));
    ...
    describe("more coverage", () => {
        it("passes the computed balance into the reconcile dialog", async () => {
            ...
            expect(dialog.querySelector<HTMLElement>(".num")!.textContent).toBe("700 ₽");
        });
        ...
        it("reconcile dialog shows a zero balance for an account with no data", async () => {
            ...
            expect(screen.getByRole("dialog").querySelector<HTMLElement>(".num")!.textContent).toBe(
                "0 ₽",
            );
        });
        ...
        it("shows no connection tag for a dangling connection id", () => {
            ...
            expect(row("Orphan").querySelectorAll<HTMLElement>(".tag")).toHaveLength(1);
            expect(
                row("Orphan").querySelector<HTMLElement>(".account-row__main")!,
        ...
        it("shows no connection tag when the account has no connection id", () => {
            ...
            expect(row("Manual").querySelectorAll<HTMLElement>(".tag")).toHaveLength(1);
        });
        ...
        it("falls back to the raw type when it has no friendly label", () => {
            ...
            expect(
                [...row("Odd").querySelectorAll<HTMLElement>(".tag")].map((t) => t.textContent),
            ).toEqual(["crypto"]);
        });
        ...
        it("labels each known account type", () => {
            ...
            const typeTag = (name: string) =>
                row(name).querySelector<HTMLElement>(".tag")!.textContent;
        ...
        it("renders the account badge at the row size", () => {
            ...
            const badge = row("Card").querySelector<HTMLElement>(".acct-badge")!;
