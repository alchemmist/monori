...
type DropTarget = { groupId: Id; index: number } | { index: number };
type CategoryDialog =
...
export default function CategoriesPage() {
    ...
    if (!snapshot) throw new Error("categories page requires a loaded snapshot");
    ...
    const cardTopsRef = useRef(new Map<string, number>());
    const colLeftsRef = useRef(new Map<string, number>());
    ...
    useLayoutEffect(() => {
        ...
        const cols = [...board.querySelectorAll<HTMLElement>(".kb-col[data-gid]")];
    ...
    const startPress = (
        ...
        info: Omit<PressInfo, "startX" | "startY">,
    ) => {
        if (e.button !== 0) return;
        if (e.pointerType !== "mouse" && e.pointerType !== "pen") return;
        if (e.target instanceof Element && e.target.closest("button, a, input, [role='menuitem']"))
            return;
        e.preventDefault();
        pressRef.current = { ...info, startX: e.clientX, startY: e.clientY };
    };

    const cardDropIndex = (colEl: HTMLElement, clientY: number) => {
        const activeId = dragRef.current?.id;
        const els = [...colEl.querySelectorAll<HTMLElement>(".kb-card")].filter(
            (el) => +String(el.dataset["id"]) !== activeId,
        );
    ...
    const updateOver = (x: number, y: number) => {
        ...
        if (d.type === "card") {
            ...
            if (!target) return;
            const cards = target.querySelector<HTMLElement>(".kb-cards");
            if (!cards) return;
            const groupId = +String(target.dataset["gid"]);
            const index = cardDropIndex(cards, y);
            ...
            const others = cols.filter((el) => +String(el.dataset["gid"]) !== d.id);
    ...
    useEffect(() => {
        const move = (e: PointerEvent) => handlersRef.current.onMove(e);
        const up = () => handlersRef.current.onUp();
        const cancel = () => handlersRef.current.endDrag();
        const key = (e: KeyboardEvent) => handlersRef.current.onKey(e);
    ...
    const notify = (toast: ToastMessage) => useStore.getState().notify(toast);
    ...
    const groupMenu = (g: CategoryGroup): RowMenuItem[] => [
...
function CardBody({ c, count, menu }: { c: Category; count: number; menu?: ReactNode }) {
...
function NewGroupControl({ onPick }: { onPick: (kind: string) => void }) {
    return (
...
function NewGroupColumn({ onPick }: { onPick: (kind: string) => void }) {
    return (
