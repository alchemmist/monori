...
import { mergeTransferRows, type TransferTableItem } from "../engine/transfers.js";
import { splitPartTransaction, type EffectiveTransaction } from "../engine/splits.js";
import "./budget.css";
import "./transfers.css";
import type { Id, Transaction } from "../types.js";
import type { SelectOption, SelectSection } from "../ui/InlineSelect.js";
...
type LedgerItem = TransferTableItem | SplitTableItem;
...
export default function TransactionsPage() {
    ...
    if (!snapshot) throw new Error("transactions page requires a loaded snapshot");
    ...
    useEffect(() => {
        if (showHidden) void loadHiddenTx();
    ...
    const acctOptionsFor = (t: Transaction | EffectiveTransaction): SelectOption[] => {
    ...
    const catSectionsFor = (
        t: Transaction | EffectiveTransaction,
    ): Array<SelectOption | SelectSection> => {
    ...
    const amountBounds = useMemo<[number, number] | null>(() => {
    ...
    const effectiveAmountRange = useMemo<[number, number] | null>(() => {
    ...
    const runTransferAction =
        (action: (transferId: string) => Promise<void>, title: string) =>
        async (transferId: string) => {
            try {
                ...
                notify({
                    title: "Failed to update the transfer",
                    theme: "danger",
                    content: String(e),
                });
            }
    ...
    const renderTxRow = (
        ...
        const transactionId =
            typeof t.id === "number" ? t.id : "parentId" in t ? t.parentId : undefined;
        if (transactionId == null) return null;
        const editable = !splitPart && isEditable(t, leg);
        const splits = t.splits ?? [];
        const sourceTransaction = snapshot.transactions.find(
                ...
                <td style={{ textAlign: "left" }} className="num">
                    {editable ? (
                        <EditableCell
                            ...
                            onCommit={(v) => {
                                if (v)
                                    void updateTransaction(transactionId, {
                ...
                >
                    {editable ? (
                        <EditableCell
                            ...
                            onCommit={(v) =>
                                void updateTransaction(transactionId, { description: v })
                            }
                        />
                    ) : (
                        t.description
                    ...
                    {splits.length > 0 && !leg && (
                        <Button
                            ...
                            size="compact-xs"
                            style={{ marginLeft: 8 }}
                            onClick={() => toggleSplit(transactionId)}
                        >
                            split · {splits.length}
                        </Button>
                    )}
                        ...
                        <EditableCell
                            ...
                            onCommit={(v) => {
                                ...
                                if (kop != null && kop !== t.amount)
                                    void updateTransaction(transactionId, { amount: kop });
                            }}
                        />
                    ) : (
                        <span className={`money num ${t.amount > 0 ? "money_pos" : ""}`}>
                    ...
                    {splitPart ? (
                      ...
                      (t.splits != null && t.splits.length > 0) ? (
                        ...
                        <InlineSelect
                            ...
                            placeholder="—"
                            value={t.categoryId != null ? String(t.categoryId) : null}
                            onChange={(v) => setTxCategory(transactionId, v ? +v : null)}
                            data={catSectionsFor(t)}
                        />
                    )}
                </td>
                                ...
                                [
                                    {
                                        ...
                                        action: () =>
                                            sourceTransaction && setDeleting(sourceTransaction),
                                        theme: "danger",
                                    },
                                ],
                            ]}
    ...
    const resetScroll =
        <T,>(fn: Dispatch<SetStateAction<T>>) =>
        (value: T) => {
