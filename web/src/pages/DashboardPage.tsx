...
import type { CategoryGroup, Id, Snapshot } from "../types.js";

type TrendRange = [number, number];
type PresetId = "6m" | "1y" | "3y" | "5y" | "ytd" | "all";
interface Preset {
    id: PresetId;
    label: string;
    months?: number;
...
const TREND_SERIES: CompositeChartSeries[] = [
...
function fmtMonthTick(key: string) {
    if (!key) return "";
    return `${MONTHS_SHORT[+key.slice(5, 7) - 1]} '${key.slice(2, 4)}`;
}

interface DonutSourceRow {
    id: Id | null;
    groupId: Id | null;
    name: string;
    total: number;
...
function donutDataFromRows(rows: DonutSourceRow[], groups: CategoryGroup[]): DonutChartCell[] {
    ...
    const named = rows.map((row) => ({
        ...row,
        label:
            (nameCount.get(row.name) ?? 0) > 1
                ? `${row.name} · ${row.groupId == null ? "Other" : groupName.get(row.groupId)}`
                : row.name,
    }));
    const top: Array<DonutSourceRow & { label: string }> = named.slice(0, 11);
    const rest = named.slice(11).reduce((sum, row) => sum + row.total, 0);
    if (rest > 0) top.push({ id: null, groupId: null, name: "Other", label: "Other", total: rest });
...
export default function DashboardPage({
    ...
    if (!snapshot) throw new Error("Dashboard requires a loaded snapshot");
    const now = useMemo(() => new Date(), []);
    const [donutYear, setDonutYear] = useState(String(now.getFullYear()));
    const [donutActive, setDonutActive] = useState<string | null>(null); // legend-hovered category name
    const [incomeDonutActive, setIncomeDonutActive] = useState<string | null>(null);
    const [allExpenseActive, setAllExpenseActive] = useState<string | null>(null);
    const [allIncomeActive, setAllIncomeActive] = useState<string | null>(null);
    ...
    const excludedIds = useMemo(() => new Set<Id>(), []);
    ...
    const monthly = useMemo(() => {
        const map = new Map<string, { income: number; expense: number }>(); // 'y-m' -> {income, expense}
        for (const t of txns) {
    ...
    const [trendRange, setTrendRange] = useState<TrendRange | null>(null); // null = default 3y
    const presetRange = (id: PresetId): TrendRange => {
        ...
        const m = PRESETS.find((p) => p.id === id)?.months ?? 0;
    ...
    const trendData = useMemo(() => {
        ...
        const rate = (j: number): number | null => {
            const entry = rows[Math.max(0, Math.min(rows.length - 1, j))];
    ...
    // The engine's category matrix nets refunds and excludes transfer legs the
    // same way the budget table does, so every chart built on it agrees with
    ...
    const scopedSnapshot: Snapshot = useMemo(
        () => ({ ...snapshot, transactions: accountTransactions }),
        [accountTransactions, snapshot],
    );
    const yearRows = (year: string) =>
        categoryYearMatrix(scopedSnapshot, year, {
            limit: Infinity,
        });
    const incomeYearRows = (year: string) =>
        categoryYearMatrix(scopedSnapshot, year, {
            limit: Infinity,
            kind: "income",
        });
    ...
    }, [donutYear, scopedSnapshot, snapshot.groups]);
    ...
    const allExpenseDonutData = useMemo(() => {
        return donutDataFromRows(categoryTotals(scopedSnapshot), snapshot.groups);
    }, [scopedSnapshot, snapshot.groups]);

    const allIncomeDonutData = useMemo(() => {
        return donutDataFromRows(
            categoryTotals(scopedSnapshot, { kind: "income" }),
            snapshot.groups,
        );
    }, [scopedSnapshot, snapshot.groups]);
    ...
    const drillData = useMemo(() => {
        ...
        const row = catId != null ? yearRows(drillYear).find((r) => r.id === catId) : null;
        const sums = row?.monthly ?? Array.from({ length: 12 }, (): number => 0);
        ...
        <div className="fade-in">
            ...
            <div className="kpi-row">
                ...
                <Kpi
                    ...
                    {...(kpis.runway != null && kpis.runway < 3
                        ? { color: "var(--m-warning)" }
                        : {})}
