...
vi.mock("../components/TransferDialog.jsx", () => ({
    default: ({ accounts, onClose }: { accounts: unknown[]; onClose: () => void }) => (
...
describe("TransactionsPage", () => {
    ...
    it("shows a neutral category for expanded transfer legs", async () => {
        ...
        for (const description of ["Transfer out", "Transfer in"]) {
            const row = screen.getByText(description).closest<HTMLElement>("tr")!;
    ...
    it("filters by year, account, category and uncategorized state", async () => {
        ...
        const toolbar = within(container.querySelector<HTMLElement>(".budget-toolbar")!);
    ...
    it("keeps archived account and category selected on legacy rows", async () => {
        ...
        renderUI(<TransactionsPage />);
        const row = screen.getByText("tx 1").closest<HTMLElement>("tr")!;
        expect(row).toHaveTextContent("Old card");
        expect(row).toHaveTextContent("Archived");
        expect(row.querySelectorAll<HTMLElement>("button.gsel")).toHaveLength(2);
    });
    ...
    it("changes an ordinary row's account and category", async () => {
        ...
        const { user } = renderUI(<TransactionsPage />);
        const row = screen.getByText("tx 1").closest<HTMLElement>("tr")!;
        const selects = row.querySelectorAll<HTMLElement>("button.gsel");
    ...
    it("opens amount range filter dropdown", async () => {
        ...
        const toolbar = container.querySelector<HTMLElement>(".budget-toolbar")!;
    ...
    it("shows hidden rows when the toggle is active", async () => {
        ...
        expect(hiddenRowEl!.closest<HTMLElement>("tr")!).toHaveClass("tx-hidden-row");
        ...
        seed({
            ...
            transactions: [
                tx(1, {
                    ...
                    splits: [
                        { id: 10, categoryId: 2, amount: -500_00, comment: "" },
                        { id: 11, categoryId: 3, amount: -300_00, comment: "" },
                    ],
                    date: "2026-03-01",
                }),
            ],
    ...
    it("renders an uncategorized transaction and allows clearing the category", async () => {
        ...
        const setTxCategory = vi.spyOn(useStore.getState(), "setTxCategory").mockResolvedValue();
        const { user } = renderUI(<TransactionsPage />);
        const row = screen.getByText("Mystery charge").closest<HTMLElement>("tr")!;
        const selects = row.querySelectorAll<HTMLElement>("button.gsel");
        await user.click(selects[1]!);
    ...
    it("shows the split indicator on a split transaction row", () => {
        seed({
            ...
            transactions: [
                tx(1, {
                    description: "Split purchase",
                    splits: [{ id: 10, categoryId: 2, amount: -500_00, comment: "" }],
                    date: "2026-03-01",
                }),
            ],
        });
    ...
    it("renders income transactions with money_pos class", () => {
        ...
        renderUI(<TransactionsPage />);
        const row = screen.getByText("Income").closest<HTMLElement>("tr")!;
        const moneyEl = row.querySelector<HTMLElement>(".money")!;
    ...
    it("offers only income categories for a positive row", async () => {
        ...
        const { user } = renderUI(<TransactionsPage />);
        const row = screen.getByText("Income tx").closest<HTMLElement>("tr")!;
        const selects = row.querySelectorAll<HTMLElement>("button.gsel");
    ...
    it("shows the delete button for an editable row", () => {
        ...
        renderUI(<TransactionsPage />);
        const row = screen.getByText("Deletable").closest<HTMLElement>("tr")!;
        expect(
            row.querySelector<HTMLElement>('[aria-label="Transaction actions"]')!,
        ).toBeInTheDocument();
    });
});
