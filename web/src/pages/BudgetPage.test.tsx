 ...
 */
function result({
    groceries = () => month(20_000_00, -5_000_00),
}: { groceries?: (month: number) => BudgetMonth } = {}): BudgetYearResult {
    return {
        year: YEAR,
        available: Array.from({ length: 12 }, (): number => 10_000_00),
        overspent: Array.from({ length: 12 }, (): number => -500_00),
        income: Array.from({ length: 12 }, (): number => 50_000_00),
        budgetedTotal: Array.from({ length: 12 }, (): number => 20_000_00),
        byCategory: new Map([[2, Array.from({ length: 12 }, (_, m) => groceries(m))]]),
    };
}

const results = (res: BudgetYearResult = result()) => new Map([[YEAR, res]]);

function render(res: BudgetYearResult = result()) {
 ...
 */
function seedWithGoal(patch: SnapshotPatch = {}) {
    const base = seed();
    return seed({
...
function goalResult(goalMonth: (month: number) => BudgetMonth = () => month(30_000_00, 0)) {
    const base = result();
    base.byCategory.set(
        4,
...
function renderGoal(res: BudgetYearResult = goalResult()) {
    return renderUI(<BudgetPage results={results(res)} firstYear={YEAR} lastYear={YEAR} />);
}
...
describe("BudgetPage", () => {
    ...
    describe("year mode", () => {
        it("renders every expense category with its yearly totals", () => {
            ...
            const totals = [...row.querySelectorAll<HTMLElement>(".yg-total")].map(
                (td) => td.textContent,
            );
        ...
        it("heads each month with its available-to-budget breakdown", () => {
            ...
            const jan = screen.getByText(`Jan ${YEAR}`).closest<HTMLElement>("th")!;
            expect(jan.querySelector<HTMLElement>(".yg-msum__av")!).toHaveTextContent("10 000 ₽");
            const breakdown = [...jan.querySelectorAll<HTMLElement>(".yg-break__line")].map(
                (l) => l.textContent,
            );
        ...
        it("celebrates the exact month an assignment brings to zero, not the current one", async () => {
            ...
            const cells = screen
                ...
                .querySelectorAll<HTMLElement>(".budget-cell");
            ...
            const msums = document.querySelectorAll<HTMLElement>(".yg-msum");
            expect(msums[0]).toHaveClass("yg-msum_complete");
            // only the edited month celebrates — not the current month, not any other
            expect(document.querySelectorAll<HTMLElement>(".yg-msum_complete")).toHaveLength(1);
        ...
        it("does not celebrate a year-grid month left with money available", async () => {
            ...
            expect(
                document.querySelector<HTMLElement>(".yg-msum_complete")!,
        ...
        it("celebrates when the remaining amount is only sub-ruble", async () => {
            ...
            const cell = screen
                .getByText("Groceries")
                .closest<HTMLElement>("tr")!
                .querySelector<HTMLElement>(".budget-cell")!;
            ...
            expect(document.querySelector<HTMLElement>(".yg-msum_complete")!).toBeInTheDocument();
        ...
        it("shows only the budgeted column in Plan density", async () => {
            ...
            expect(document.querySelectorAll<HTMLElement>(".yg-metric")).toHaveLength(36);

            await user.click(screen.getByText("Plan"));

            const labels = [...document.querySelectorAll<HTMLElement>(".yg-metric")].map(
                (th) => th.textContent,
            );
        ...
        it("saves a year-grid budget edit against the month it sits in", async () => {
            ...
            const input = document.querySelector<HTMLElement>(".budget-cell__input")!;
            await user.clear(input);
        ...
        it("hides the month picker and month-only hero cards in year mode", () => {
            ...
            expect(document.querySelector<HTMLElement>(".hero-card")!).toBeNull();
        ...
        it("orders expense groups ahead of goal groups", () => {
            seedWithGoal();
            renderGoal();
            const names = [...document.querySelectorAll<HTMLElement>(".yg-group .yg-name")].map(
    ...
    describe("month mode", () => {
        const toMonth = async (user: UserEvent, label = "Mar") => {
        ...
        it("shows the month's hero metrics", async () => {
            ...
            const hero = (label: string) =>
                [...document.querySelectorAll<HTMLElement>(".hero-card")].find(
                    (c) => c.querySelector<HTMLElement>(".hero-card__label")!.textContent === label,
                );
        ...
        it("totals a group row from the categories under it", async () => {
            ...
            const group = screen.getByText("Living").closest<HTMLElement>("tr")!;
            const cells = [...group.querySelectorAll<HTMLElement>("td")].map(
                (td) => td.textContent,
            );
        ...
        it("saves a monthly budget edit against the selected month", async () => {
            ...
            const row = screen.getByText("Groceries").closest<HTMLElement>("tr")!;
            await user.click(row.querySelector<HTMLElement>(".budget-cell")!);
            const input = row.querySelector<HTMLElement>(".budget-cell__input")!;
            await user.clear(input);
            await user.type(input, "300");
            await user.keyboard("{Enter}");
        ...
        it("celebrates only when a user assignment brings available to exactly zero", async () => {
            ...
            const hero = screen
                .getByText("Available to budget")
                .closest<HTMLElement>(".hero-card")!;
            ...
            expect(hero.querySelector<HTMLElement>(".hero-card__label")!).toHaveTextContent(
                "Available to budget",
            );
        ...
        it("celebrates when monthly Available ends with only sub-ruble remainder", async () => {
            ...
            expect(
                row
                    ...
                    .querySelector<HTMLElement>(".hero-card_complete")!,
            ).toBeInTheDocument();
        ...
        it("does not celebrate an already-zero budget on page load", async () => {
            const res = result();
            res.available = Array.from({ length: 12 }, (): number => 0);
            const { user } = render(res);
            await toMonth(user);

            expect(
                screen.getByText("Available to budget").closest<HTMLElement>(".hero-card")!,
            ).not.toHaveClass("hero-card_complete");
        });
        ...
        it("caps the spent bar at the full width once the envelope is emptied", async () => {
            ...
            const fill = screen
                .getByText("Groceries")
                .closest<HTMLElement>("tr")!
                .querySelector<HTMLElement>(".cat-progress__fill")!;
        ...
        it("opens the delete form for a category from its row menu", async () => {
            ...
            const row = screen.getByText("Groceries").closest<HTMLElement>("tr")!;
            await user.click(row.querySelector<HTMLElement>(".cat-row__menu button")!);
        ...
        it("colors the available card as income when positive and shows the month end", async () => {
            const { user } = render();
            await toMonth(user);
            const avail = [...document.querySelectorAll<HTMLElement>(".hero-card")].find(
                (c) =>
                    c.querySelector<HTMLElement>(".hero-card__label")!.textContent ===
                    "Available to budget",
            );
            const value = avail!.querySelector<HTMLElement>(".hero-card__value")!;
        ...
        it("colors the available card as expense when negative", async () => {
            const res = result();
            res.available = Array.from({ length: 12 }, (): number => -10_000_00);
            const { user } = render(res);
            await toMonth(user);
            const avail = [...document.querySelectorAll<HTMLElement>(".hero-card")].find(
                (c) =>
            ...
            expect(avail!.querySelector<HTMLElement>(".hero-card__value")!).toHaveStyle({
                color: "var(--m-expense)",
            });
        });
        ...
        it("colors overspent red only when below zero, faint otherwise", async () => {
            ...
            const overspent = () =>
                [...document.querySelectorAll<HTMLElement>(".hero-card")]
                    ...
                    .querySelector<HTMLElement>(".hero-card__value")!;
            ...
            seed();
            const res = result();
            res.overspent = Array.from({ length: 12 }, (): number => 0);
    ...
    describe("goals", () => {
        ...
        it("offers Distribute across months when the goal has a target", async () => {
            ...
            const { user } = renderGoal();
            await toMonth(user);
            const row = screen.getByText("Trip").closest<HTMLElement>("tr")!;
            await user.click(row.querySelector<HTMLElement>(".cat-row__menu button")!);
            expect(
    ...
    describe("GoalUrgency", () => {
        ...
        const withDate = (date: string | null) =>
            seedWithGoal({
                categories: [
                    ...seed().categories,
                    {
        ...
        const openTooltip = async (user: UserEvent) => {
            ...
            return funded.closest<HTMLElement>(".goal-label__popup")!;
        };
    ...
    describe("DistributeGoalDialog", () => {
        ...
        const openDistribute = async (user: UserEvent) => {
            await toMonth(user);
            const row = screen.getByText("Trip").closest<HTMLElement>("tr")!;
            await user.click(row.querySelector<HTMLElement>(".cat-row__menu button")!);
            await user.click(
