...
describe("SettingsPage", () => {
    ...
    it("shows account, appearance and data controls for a signed-in user", () => {
        useStore.setState({
            user: { email: "me@example.com", createdAt: "2026-01-02", isAdmin: true },
        });
        const toggle = vi.fn();
    ...
    it("shows no join date or admin badge for a plain account", () => {
        useStore.setState({ user: { email: "plain@example.com" } });
    ...
    it("downloads the exported workbook and reports an API error", async () => {
        ...
        api.exportXlsx.mockResolvedValueOnce(blob);
        const { user } = renderUI(
        ...
        expect(click.mock.instances[0]).toHaveAttribute("download", "monori-export.xlsx");
        expect(click.mock.instances[0]).toHaveAttribute("href", "blob:test");
