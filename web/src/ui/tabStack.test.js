...
describe("tab registry", () => {
    ...
    it("notifies on resizeTab even if width stays the same", () => {
        const fn = vi.fn();
        mount("a");
        const unsub = subscribe(fn);
        resizeTab("a", TAB_WIDTH);
        expect(fn).toHaveBeenCalledTimes(1);
        unsub();
    });

    it("notifies on unregisterTab", () => {
        mount("a");
        const fn = vi.fn();
        subscribe(fn);
        unregisterTab("a");
    ...
    it("allows multiple registrations and unregistrations", () => {
        mount("a");
        mount("b");
        mount("c");
        expect(offsetOf("a")).toBe(0);
        expect(offsetOf("b")).toBe(TAB_WIDTH);
        expect(offsetOf("c")).toBe(TAB_WIDTH * 2);
    ...
    it("handles resizing to smaller widths", () => {
        mount("a", 500);
        mount("b", 400);
        expect(offsetOf("b")).toBe(500);

        resizeTab("a", TAB_STRIP_WIDTH);
        expect(offsetOf("b")).toBe(TAB_STRIP_WIDTH);
    });

    it("handles resizing to larger widths", () => {
        mount("a", TAB_STRIP_WIDTH);
        mount("b", TAB_WIDTH);
        expect(offsetOf("b")).toBe(TAB_STRIP_WIDTH);

        resizeTab("a", 600);
        expect(offsetOf("b")).toBe(600);
    });

    it("subscriber unsubscribe returns a function", () => {
        const fn = vi.fn();
        const unsub = subscribe(fn);
    ...
    it("exports TAB_WIDTH and TAB_STRIP_WIDTH constants", () => {
        expect(TAB_WIDTH).toBe(420);
        expect(TAB_STRIP_WIDTH).toBe(34);
    });

    it("handles deeply nested offset calculations", () => {
        for (let i = 0; i < 10; i++) {
            mount(String(i), 100);
        }
        expect(offsetOf("5")).toBe(500);
        expect(offsetOf("9")).toBe(900);
    });
});
