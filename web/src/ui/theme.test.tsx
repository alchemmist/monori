...
describe("theme", () => {
    ...
    it("counter-scales the 14px root font so Mantine rems land on their px values", () => {
        wrap(<div />);
        const scale =
            document.querySelector<HTMLElement>("style[data-mantine-styles]")?.textContent ?? "";
        expect(scale).toContain(`--mantine-scale: ${16 / 14}`);
    });

    it("rounds every surface to the app's own radius rather than Mantine's", () => {
        wrap(<div />);
        const styles = document.querySelector<HTMLElement>(
    ...
    it("gives the whole app a pointer cursor on interactive controls", () => {
        ...
        expect(
            document.querySelector<HTMLElement>("style[data-mantine-styles]")!.textContent,
        ).toContain("--mantine-cursor-type: pointer");
    });
    ...
    it("keeps a select from deselecting and puts the check on the right", async () => {
        ...
        const chosen = await waitFor(() => {
            const el = document.querySelector<HTMLElement>('[role="option"][value="One"]')!;
    ...
    it("renders the whole option list inline instead of inside a scroll area", async () => {
        ...
        await waitFor(() =>
            expect(document.querySelectorAll<HTMLElement>('[role="option"]')).toHaveLength(2),
        );
        expect(document.querySelector<HTMLElement>(".mantine-ScrollArea-root")!).toBeNull();
    });

    it("centres modals and dims the page behind them", () => {
        wrap(<Modal opened onClose={() => {}} title="Hi" />);
        expect(screen.getByText("Hi")).toBeInTheDocument();
        expect(document.querySelector<HTMLElement>(".mantine-Modal-root")!).toHaveAttribute(
            "data-centered",
            "true",
        );
        expect(
            document
                .querySelector<HTMLElement>(".mantine-Modal-overlay")!
                .style.getPropertyValue("--overlay-bg"),
        ).toBe("rgba(0, 0, 0, 0.5)");
    });
    ...
    it("drops the tooltip arrow", async () => {
        ...
        expect(document.querySelector<HTMLElement>(".mantine-Tooltip-arrow")!).toBeNull();
    });
});
