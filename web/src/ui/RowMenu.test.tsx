...
const sizeVar = (button: HTMLElement) => button.style.getPropertyValue("--ai-size");

/** Open the menu and hand back its dropdown element. */
async function open(user: UserEvent) {
    await user.click(screen.getByRole("button"));
    await waitFor(() =>
        expect(document.querySelector<HTMLElement>('[role="menu"]')!).toBeInTheDocument(),
    );
...
describe("RowMenu", () => {
    ...
    it("maps each size alias onto its own pixel size", () => {
        const cases: Array<["xs" | "s" | "m", string]> = [
    ...
    it("falls back to the ellipsis icon when no icon is given", () => {
        ...
        expect(container.querySelectorAll<HTMLElement>("svg")).toHaveLength(1);
    });
    ...
    it("puts a divider between groups and never before the first", async () => {
        ...
        expect(menu.querySelectorAll<HTMLElement>(".mantine-Menu-divider")).toHaveLength(2);

        // and they sit at the group seams, in reading order
        const rendered = [
            ...menu.querySelectorAll<HTMLElement>('[role="menuitem"], .mantine-Menu-divider'),
        ].map((el) => (el.getAttribute("role") === "menuitem" ? el.textContent : "---"));
    ...
    it("draws no divider for a flat list, however many items it holds", async () => {
        ...
        const menu = await open(user);
        expect(menu.querySelectorAll<HTMLElement>(".mantine-Menu-divider")).toHaveLength(0);
        expect(
    ...
    it("renders a grouped list differently from the same items flat", async () => {
        ...
        expect(flatMenu.querySelectorAll<HTMLElement>(".mantine-Menu-divider")).toHaveLength(0);
        flat.unmount();

        const grouped = renderUI(<RowMenu items={[[items[0]!], [items[1]!]]} />);
        const groupedMenu = await open(grouped.user);
        expect(groupedMenu.querySelectorAll<HTMLElement>(".mantine-Menu-divider")).toHaveLength(1);
        // same items either way, only the separation differs
        expect(
            [...groupedMenu.querySelectorAll<HTMLElement>('[role="menuitem"]')].map(
                (el) => el.textContent,
            ),
        ).toEqual(["Edit", "Delete"]);
    });
