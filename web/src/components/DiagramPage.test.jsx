...
describe("DiagramPage", () => {
    it("renders the selected diagram, fits it to the stage and links back", () => {
        useMermaidSvg.mockReturnValue({ svg: '<svg viewBox="0 0 800 400"></svg>', failed: false });
        ...
        expect(container.querySelector(".diagram-page__canvas")).toHaveStyle({
            width: "800px",
            height: "400px",
    ...
    it("zooms through buttons, keyboard, wheel and restores fitted view", () => {
        useMermaidSvg.mockReturnValue({ svg: "<svg></svg>", failed: false });
        const { container } = renderDiagram();
    ...
    it("explains missing and failed diagrams", () => {
        useMermaidSvg.mockReturnValue({ svg: "", failed: false });
        ...
        useMermaidSvg.mockReturnValue({ svg: "", failed: true });
        unmount();
