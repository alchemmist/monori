...
describe("Landing", () => {
    it("offers the product entry points and documentation", () => {
        renderUI(<Landing />, { wrapper: ({ children }) => <MemoryRouter>{children}</MemoryRouter> });
        expect(screen.getByRole("heading", { name: /your budget/i })).toBeInTheDocument();
        expect(screen.getByRole("link", { name: /sign in/i })).toHaveAttribute("href", "/login");
        expect(screen.getByRole("link", { name: /try the live demo/i })).toHaveAttribute("href", "/demo");
        expect(screen.getAllByRole("link", { name: /get started/i })[0]).toHaveAttribute("href", "/docs/getting-started");
