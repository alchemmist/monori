...
export default function TimeNavigator({
    ...
    const areaPath = useMemo(() => {
        ...
        const cy = (value: number) => Math.max(top, Math.min(bottom, value));
        const [x0, y0] = pt(0);
    ...
    const yearTicks = useMemo(() => {
        const ticks: Array<{ i: number; year: string }> = [];
        for (let i = 0; i < n; i++) {
    ...
    const clampRange = (a: number, b: number): TimeRange => {
        const span = b - a;
        const na = Math.max(0, Math.min(a, n - 1 - span));
        return [na, na + span];
    };

    const startDrag = (mode: DragMode) => (e: ReactPointerEvent<SVGElement>) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const onTrackDown = (e: ReactPointerEvent<SVGSVGElement>) => {
        if (!cellW) return;
