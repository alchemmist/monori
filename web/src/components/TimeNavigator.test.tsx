...
describe("TimeNavigator", () => {
    ...
    it("draws the history, year marks and selected window", () => {
        ...
        expect(document.querySelector<HTMLElement>(".timenav__area")!).toHaveAttribute(
            "d",
        ...
        const windowRect = document.querySelector<HTMLElement>(".timenav__window")!;
        expect(windowRect).toHaveAttribute("x", "240");
        expect(windowRect).toHaveAttribute("width", "720");
        const [before, after] = document.querySelectorAll<HTMLElement>(".timenav__dim");
    ...
    it("moves and resizes the range through pointer gestures", () => {
        ...
        fireEvent.pointerDown(document.querySelectorAll<HTMLElement>(".timenav__handle")[0]!, {
            pointerId: 2,
            clientX: 300,
        });
    ...
    it("centres the window when clicking the overview track", () => {
        ...
        fireEvent.pointerDown(document.querySelector<HTMLElement>("svg")!, {
            pointerId: 1,
            clientX: 1100,
        });
    ...
    it("does not let either resize handle shrink the window below three months", () => {
        ...
        renderUI(<TimeNavigator items={items} range={[1, 3]} onChange={onChange} />);
        const svg = document.querySelector<HTMLElement>("svg")!;
        const [left, right] = document.querySelectorAll<HTMLElement>(".timenav__handle");
    ...
    it("does not render an SVG for an empty history", () => {
        ...
        expect(document.querySelector<HTMLElement>("svg")!).not.toBeInTheDocument();
    });
});
