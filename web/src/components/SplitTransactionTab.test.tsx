...
const expense = (patch: Partial<Transaction> = {}) =>
    tx(42, { amount: -1_000, description: "Market", splits: [], ...patch });
...
describe("SplitTransactionTab", () => {
    ...
    const renderDialog = (transaction: Transaction | null = expense(), onClose = vi.fn()) => ({
        onClose,
        ...renderUI(<SplitTransactionTab transaction={transaction} onClose={onClose} />),
    });

    const chooseCategories = async (user: UserEvent) => {
