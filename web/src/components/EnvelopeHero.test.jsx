...
describe("EnvelopeHero", () => {
    ...
    it("applies is-hot class to Eating out envelope", () => {
        const { container } = renderUI(<EnvelopeHero />);
        const eatingOut = Array.from(container.querySelectorAll(".env-hero__env")).find(
            (el) => el.textContent.includes("Eating out"),
        );
    ...
    it("does not apply is-hot class to non-hot envelopes", () => {
        const { container } = renderUI(<EnvelopeHero />);
        const groceries = Array.from(container.querySelectorAll(".env-hero__env")).find(
            (el) => el.textContent.includes("Groceries"),
        );
