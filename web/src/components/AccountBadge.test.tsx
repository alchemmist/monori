...
function glyphOf(account: Partial<Account>, props: { size?: number } = {}) {
    const { container, unmount } = renderUI(<AccountBadge account={account} {...props} />);
    const svg = container.querySelector<HTMLElement>(".acct-badge svg")!.innerHTML;
    unmount();
    return svg;
}
...
describe("AccountBadge", () => {
    ...
    it("is 30px square by default and scales the glyph to 56% of the tile", () => {
        ...
        const badge = container.querySelector<HTMLElement>(".acct-badge")!;
        expect(badge).toHaveStyle({ width: "30px", height: "30px" });
        const svg = badge.querySelector<HTMLElement>("svg")!;
    ...
    it("shows an uploaded image instead of any glyph or tint", () => {
        ...
        expect(badge.querySelector<HTMLElement>("img")!).toHaveAttribute("src", imageUrl);
        expect(badge.querySelector<HTMLElement>("svg")!).toBeNull();
        expect(badge.style.background).toBe("");
    });
});
