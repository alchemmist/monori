...
describe("MigratePanel", () => {
    ...
    it("previews a workbook, requires every marker mapping, and commits the selected policy", async () => {
        ...
        fireEvent.change(container.querySelector<HTMLElement>('input[type="file"]')!, {
            target: { files: [file] },
        });
        ...
        const selectors = container.querySelectorAll<HTMLElement>("button.gsel");
    ...
    it("defaults the conflicting budget cells to being overwritten", async () => {
        ...
        await user.click(container.querySelector<HTMLElement>("button.gsel")!);
        await user.click(await screen.findByText("Card"));
        await user.click(screen.getByRole("button", { name: "Import" }));
        await waitFor(() =>
    ...
    it("offers only same-currency accounts and flags currencies with none", async () => {
        ...
        await user.click(container.querySelectorAll<HTMLElement>("button.gsel")[0]!);
    ...
    it("preselects the settings default account for the unmarked slot", async () => {
        ...
        useStore.setState({ user: { id: 1, email: "me@example.com", defaultAccountId: 1 } });
        vi.spyOn(api, "workbookPreview").mockResolvedValue({
            groups: 0,
            categories: 0,
            transactions: 3,
