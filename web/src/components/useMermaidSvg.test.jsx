...
describe("useMermaidSvg", () => {
    ...
    it("configures Mermaid for the current scheme and renders its SVG", async () => {
        ...
        expect(mermaid.render).toHaveBeenCalledWith(expect.stringMatching(/^mermaid-[A-Za-z0-9]+-1$/), "flowchart LR");
    });

    it("uses Mermaid's dark theme and re-renders when either input changes", async () => {
        useComputedColorScheme.mockReturnValue("dark");
        mermaid.render.mockResolvedValueOnce({ svg: "<svg>one</svg>" }).mockResolvedValueOnce({ svg: "<svg>two</svg>" });
    ...
    it("reports render errors and clears the error when a new render starts", async () => {
        mermaid.render.mockRejectedValueOnce(new Error("bad chart")).mockResolvedValueOnce({ svg: "<svg>fixed</svg>" });
        const { rerender } = renderUI(<Probe chart="bad" />);
        await waitFor(() => expect(screen.getByRole("status")).toHaveAttribute("data-failed", "true"));
