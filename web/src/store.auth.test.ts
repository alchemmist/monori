...
const tab = (id: number, key: string | null, kind = "note"): TabDescriptor => ({
    id,
    key,
    kind,
    props: {},
...
describe("checkAuth", () => {
    ...
    it("drops the stale token and the tabs when the token no longer resolves", async () => {
        ...
        useStore.setState({ user: { id: 3, email: "a@b.c" }, tabs: [tab(1, "a")] });
...
describe("login, register and logout", () => {
    ...
    it("registers then logs the new account straight in", async () => {
        const register = vi
            .spyOn(api, "authRegister")
            .mockResolvedValue({ id: 10, email: "new@b.c" });
    ...
    it("wipes the token, the user and the tabs on logout", () => {
        ...
        useStore.setState({ user: { id: 9, email: "a@b.c" }, tabs: [tab(1, "a"), tab(2, "b")] });
