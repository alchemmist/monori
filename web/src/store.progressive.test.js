...
describe("progressive transaction loading", () => {
    ...
    it("throws away the page a superseded fill was still holding", async () => {
        ...
        useStore.setState({
            snapshot: { transactions: [tx(1)], transactionsTotal: 3 },
            loading: false,
            txProgress: null,
        });
...
describe("load", () => {
    ...
    it("discards a light snapshot whose load was already superseded", async () => {
        const first = { transactions: [tx(1)], transactionsTotal: 1, tag: "first" };
        const second = { transactions: [tx(2)], transactionsTotal: 1, tag: "second" };
        ...
        expect(useStore.getState().snapshot.tag).toBe("second");
    });
...
describe("fillTransactions", () => {
    ...
    it("keeps the true row count when the server under-reports the total", async () => {
        ...
        useStore.setState({
            snapshot: { transactions: local, transactionsTotal: 2 },
            loading: false,
        });
    ...
    it("stops on a short page even while the reported total is still higher", async () => {
        ...
        useStore.setState({
            snapshot: { transactions: [], transactionsTotal: 10_000 },
            loading: false,
            toast: null,
        });
    ...
    it("lands a mid-fill flush once the chunks outlive the flush window", async () => {
        ...
        useStore.setState({
            snapshot: { transactions: [], transactionsTotal: TX_CHUNK * 2 },
            loading: false,
        });
        const writes = [];
    ...
    it("stops once the loaded rows reach the total on a full page", async () => {
        ...
        useStore.setState({
            snapshot: { transactions: [], transactionsTotal: TX_CHUNK },
            loading: false,
        });
