...
const openSplitEditor = async (page, description, action = "Split transaction") => {
...
const choosePartCategory = async (page, part, category) => {
...
test("split journey keeps allocation, budgets and dashboard visualizations consistent", async ({
    ...
    expect([...expense.splits, ...income.splits].every((part) => part.accountId == null)).toBe(
        true,
    );
