...
| `MONORI_ADMIN_EMAILS` | *(unset)* | Comma-separated emails that get admin rights (the in-app Admin panel: instance analytics, user management and the SQL console). The flag is synced on every login, so this variable is the single source of truth — remove an email and the rights are revoked at the next login. |
