...
| `make mutation`      | Full mutation sweep: Stryker on `web/src`, mutmut on `server/app`.                                                                             |
| `make mutation-diff` | Diff-scoped mutation gate for changed frontend files and backend functions (`BASE=origin/main`, threshold 90%).                               |
