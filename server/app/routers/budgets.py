...
def _set_cell(c, cell: BudgetCell):
    if cell.amount == 0:
        c.execute(
...
@router.post("/bulk")
def bulk_budgets(body: BulkBody):
    c = conn()
    try:
...
@router.post("/copy")
def copy_budgets(body: CopyBody):
    ...
    try:
        if month_mode:
            src = c.execute(
                ...
                (body.fromYear, body.fromMonth),
            ).fetchall()
            c.execute("DELETE FROM budgets WHERE year=? AND month=?", (body.toYear, body.toMonth))
            for r in src:
                c.execute(
                    "INSERT INTO budgets (category_id, year, month, amount) VALUES (?, ?, ?, ?)",
            ...
            src = c.execute(
                "SELECT category_id, month, amount FROM budgets WHERE year=?", (body.fromYear,)
            ).fetchall()
            c.execute("DELETE FROM budgets WHERE year=?", (body.toYear,))
            for r in src:
                c.execute(
                    "INSERT INTO budgets (category_id, year, month, amount) VALUES (?, ?, ?, ?)",
