...
def test_light_snapshot_returns_newest_window_in_canonical_order(api, client):
...
def test_light_snapshot_plus_paged_fill_reconstructs_the_full_ledger(api, client):
...
def test_snapshot_without_light_is_unpaged(api, client):
...
def test_light_snapshot_shorter_than_the_window_reports_its_own_total(api, client):
...
def test_light_snapshot_limit_is_bounded(client):
