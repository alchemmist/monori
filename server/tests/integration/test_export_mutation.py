...
def test_year_sheet_sums_repeated_category_rows(api, client):
    _rich(api, client)
...
def test_month_summary_sums_across_categories(api, client):
    _rich(api, client)
    ws = _export(client)["2026"]
...
def test_year_sheet_uses_a_three_wide_month_stride(api, client):
    _rich(api, client)
    ws = _export(client)["2026"]
    row = _row_of(ws, "Groceries")
...
def test_year_sheet_money_cells_keep_the_number_font(api, client):
    _rich(api, client)
    ws = _export(client)["2026"]
    row = _row_of(ws, "Groceries")
...
def test_dashdata_sums_income_across_rows(api, client):
    _rich(api, client)
...
def test_dashdata_carries_a_running_cumulative_net(api, client):
    _rich(api, client)
    ws = _export(client)["DashData"]
...
def test_dashdata_leaves_the_ratio_blank_without_income(api, client):
    _rich(api, client)
    ws = _export(client)["DashData"]
...
def test_transactions_sheet_is_frozen_below_the_header(api, client):
    _rich(api, client)
