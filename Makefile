...
.PHONY: install setup tools dev down reset-db deploy api web build clean \
        ...
        test t-fast t-medium t-ci t-ci-unit t-ci-integration t-slow t-slow-ui t-front t-back t-e2e t-e2e-ui coverage coverage-baseline coverage-diff perf-front-diff mutation mutation-diff mutation-python m-front m-front-diff m-front-file m-back m-back-diff \
...
clean:
	...
	for path in \
		.coverage coverage.json coverage.xml htmlcov coverage coverage-baseline coverage-report \
