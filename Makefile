...
.PHONY: install setup tools dev down reset-db deploy api web build clean \
        ...
        test t-workflow t-fast t-medium t-ci t-ci-unit t-slow t-slow-ui t-front t-back t-e2e t-e2e-ui coverage coverage-baseline coverage-diff perf-front-diff mutation mutation-diff mutation-python m-front m-front-diff m-front-file m-back m-back-diff \
...
docs-links:
	@git ls-files '*.md' | lychee --config .lychee.toml --files-from -
...
spell:
	uvx codespell --skip='*/mutants/*,server/data/*' web/src $(PYTHON_SOURCES) \
...
time-bombs:
	uv run --locked python -m monori.ci.lib.time_bombs --base "$(BASE)"

test: t-front t-back t-e2e

t-workflow:
	env -u GITHUB_STEP_SUMMARY -u MUTATION_SUMMARY_PATH uv run --locked pytest -q ci/tests/test_main_workflow.py ci/tests/test_performance_workflow.py ci/tests/test_public_quality_graph.py
...
t-ci-unit:
	env -u GITHUB_STEP_SUMMARY -u MUTATION_SUMMARY_PATH uv run --locked --group test pytest -q ci/tests
